...
func InitRouter(db *gorm.DB) {
	...
	{
		paper_detail.GET("/pressureundo", ctrl.HandleGetPaperDetailsWithPressureUndo)
		paper_detail.GET("/notpressureundo", ctrl.HandleGetPaperDetailsWithNotPressureUndo)
	...
	stroke := e.Group("/strokes")
	{
		...
		stroke.PUT("/:pdid", ctrl.HandleUpdateStroke)
